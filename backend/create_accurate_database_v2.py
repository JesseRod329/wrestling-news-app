#!/usr/bin/env python3
"""
Create Accurate Wrestling Database V2
Improved parsing logic to handle HTML content properly
"""
...
class AccurateWrestlerDataFetcherV2:
    ...
    def fetch_wrestler_profile(self, wrestler_id: str, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"      Age: {wrestler_data.get('age', 'N/A')}, Promotion: {wrestler_data.get('promotion', 'N/A')}")
    ...
    def _parse_profile_page_v2(self, soup: BeautifulSoup, name: str) -> Dict[str, Any]:
        """Parse the profile page HTML with improved logic"""
        data = {}
        ...
        try:
            ...
            for table in info_tables:
                ...
                for row in rows:
                    ...
                    if title_div and content_div:
                        title = title_div.get_text(strip=True).rstrip(':')  # Remove trailing colon
                        ...
                        if title == "Current gimmick":
                            ...
                            data["brand"] = content
                        elif title == "Active Roles" or title == "Roles":
                            data["roles"] = [content]
                        elif title == "Birthplace" or title == "Hometown":
                        ...
                        elif title == "Beginning of in-ring career" or title == "Debut":
                            data["debut"] = content
                        elif title == "In-ring experience" or title == "Experience":
            ...
            # Try to find real name from the subtitle
            subtitle_elem = soup.find('h2', {'class': 'TextSubHeader'})
            if subtitle_elem:
                subtitle_text = subtitle_elem.get_text(strip=True)
                if subtitle_text.startswith('Also known as'):
                    real_names = subtitle_text.replace('Also known as ', '').split(', ')
                    if real_names:
                        data["real_name"] = real_names[0]  # First name is usually the real name
            
            # Try to find social media links
            social_media = {}
            social_links = soup.find_all('a', href=re.compile(r'(twitter\.com|x\.com|instagram\.com|tiktok\.com|youtube\.com|facebook\.com)'))
    ...
    def _parse_stats_page_v2(self, soup: BeautifulSoup) -> Dict[str, Any]:
        """Parse the stats page HTML with improved logic"""
        data = {}
        
        try:
            # Look for rating information in the ratings box
            rating_elem = soup.find('div', {'class': 'RatingsBoxAdjustedRating'})
            if rating_elem:
                try:
                    rating_text = rating_elem.get_text(strip=True)
                    data["averageRating"] = float(rating_text)
                except ValueError:
                    pass
            
            # Look for vote count
            votes_elem = soup.find('div', string=re.compile(r'Valid votes:', re.IGNORECASE))
            if votes_elem:
                votes_text = votes_elem.get_text(strip=True)
                votes_match = re.search(r'Valid votes:\s*(\d+)', votes_text, re.IGNORECASE)
                if votes_match:
                    data["total_votes"] = int(votes_match.group(1))
            
            # Look for comment count
            comments_elem = soup.find('div', string=re.compile(r'Number of comments:', re.IGNORECASE))
            if comments_elem:
                comments_text = comments_elem.get_text(strip=True)
                comments_match = re.search(r'Number of comments:\s*(\d+)', comments_text, re.IGNORECASE)
                if comments_match:
    ...
    def save_database(self, wrestlers: List[Dict[str, Any]]) -> None:
        ...
        filename = f"wrestling_database_accurate_final_v2_{timestamp}.json"
        ...
        database = {
            "metadata": {
                ...
                "description": "Comprehensive accurate wrestling database with correct Cagematch IDs (V2 - Improved Parsing)",
                "data_source": "Cagematch.net",
                "data_quality": "High - Verified IDs and accurate parsing with HTML handling"
            },
            "wrestlers": wrestlers
        }
        ...
        mapping_filename = f"wrestler_id_mapping_final_v2_{timestamp}.json"
...
def main():
    ...
    print("🎯 Creating Accurate Wrestling Database V2")
    print("=" * 80)
    ...
    if wrestlers:
        ...
        for wrestler in wrestlers:
            age = wrestler.get('age', 'N/A')
            promotion = wrestler.get('promotion', 'N/A')
            print(f"✅ {wrestler['name']}: {age}, {promotion}")
